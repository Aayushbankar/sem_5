---
title: "P06 — Mvp Feature Prioritization"
sidebar:
  order: 6
---
...
- Template file: [`p06_prioritization_matrix_template.md`](./p06_prioritization_matrix_template.md.md)
...
> When running **Mvp Feature Prioritization** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Feature source** — every feature must trace back to a persona goal or a journey pain point (P04). Orphan features are scope creep.
- **MVP ≠ smallest possible** — it's the smallest set that *proves the value hypothesis*. If Riya won't re-use StudyMate after F1+F3+F5, the concept fails — and that's what an MVP is for: fail fast, cheaply.
- **What is an MVP?** — The minimum feature set that delivers core value to early users and lets you test the riskiest assumptions with the least effort.
- **Why are F1/F3/F5 the MVP and not F2?** — They are quick wins with the same core value (convert material → testable understanding); F2 needs heavier RAG plumbing, so it ships a sprint later.
- **What's a "money pit" feature?** — High effort, low impact (F11 dashboards) — cut, because it consumes budget the MVP can't afford.
- **How does the matrix link to P04?** — Impact scores come from the journey's pain-point frequency/intensity; a feature nobody's pain demanded scores low.
