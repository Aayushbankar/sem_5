---
title: "P05 — Pir Ldr Sensor Interface"
sidebar:
  order: 5
---
...
> [!warning] Pin rule
> use an **ADC1 input-only pin** (GPIO 34/35/36/39) for the LDR so the ADC is not shared with the Wi-Fi radio.
...
> Full sketch: [`p05_pir_ldr_sensor.ino`](./p05_pir_ldr_sensor.ino.md)
...
> When running **Pir Ldr Sensor Interface** in a real environment, it almost never works perfectly the first time. 
> 
...
- **L (single/non-repeatable)** — move the jumper to H for continuous re-trigger while someone is present.
- **Not an actual run** — expected behaviour, not captured output. On hardware, wave a hand in front of the PIR and cover/uncover the LDR:
- **What does PIR stand for?** — Passive Infrared; it detects infrared radiation changes, not visible motion.
- **Why is the LDR read through a resistor divider?** — The ADC reads voltage, not resistance; the divider converts resistance change to voltage change.
- **Why GPIO 34 for analog?** — Input-only ADC1 pin; keeps the ADC away from Wi-Fi interference and avoids boot-constraint pins.
- **What is the ADC resolution?** — 12-bit → 0–4095 (0–3.3 V).
- **What do the two PIR potentiometers adjust?** — Sensitivity (distance) and hold time (delay).
- **Is the LDR reading linear?** — No — approximate/logarithmic; we only use it as a relative brightness indicator.
