---
title: "P06 — Dht Temperature Humidity Sensor"
sidebar:
  order: 6
---
...
> [!warning] DHT22 caution
> the sensor is **very slow** (needs ≥2 s between reads). If you sample faster it returns `NaN` — always `delay(2000)` between reads.
...
> Full sketch: [`p06_dht_sensor.ino`](./p06_dht_sensor.ino.md)
...
> When running **Dht Temperature Humidity Sensor** in a real environment, it almost never works perfectly the first time. 
> 
...
- **proprietary 1-wire-like protocol** — after the host pulls the data line low for ~18 ms, the sensor replies with 40 bits: `8× humidity integer + 8× humidity decimal + 8× temp integer + 8× temp decimal + 8× checksum`.
- **Not an actual run** — representative expected values for a room (~27 °C):
- **Which libraries are needed?** — *DHT sensor library by Adafruit* and its dependency *Adafruit Unified Sensor*.
- **How many data bits does DHT send?** — 40 bits (16 humidity + 16 temperature + 8 checksum).
- **Why the pull-up resistor on DATA?** — The DHT uses open-drain signalling; the pull-up defines the idle HIGH level.
- **Why does DHT need 2 s between reads?** — Its internal conversion/update rate is ~0.5–1 Hz; faster reads return NaN.
- **DHT11 accuracy?** — ±2 °C / ±5 %RH; DHT22 is ±0.5 °C / ±2 %RH.
- **What does `isnan()` check?** — That the read returned a valid number (not Not-a-Number).
