---
title: "P08 — Cloudsim Secure File Sharing"
sidebar:
  order: 8
---
...
> CloudSim's simulator needs Java + the CloudSim 3.0.3 jar. This machine has **Java 21** (compatible) but the CloudSim jar is not bundled with the repo — compile/run **in the lab** using the steps below.
...
## 4. The simulation
Full source: [`p08_cloudsim_secure_file_sharing.java`](./p08_cloudsim_secure_file_sharing.java.md)
...
> When running **Cloudsim Secure File Sharing** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Interpretation:** — Each cloudlet = one step of the sharing workflow, all **SUCCESS**.
- **What is CloudSim?** — An open-source Java framework for simulating cloud datacenters, VMs, and workload scheduling.
- **What are cloudlets?** — Tasks/units of work submitted to VMs (length in MI).
- **Does CloudSim encrypt data?** — No; it simulates resources. Security is modeled at the application layer.
- **What does the broker do?** — Acts as the tenant/account: submits VMs and cloudlets to datacenters.
- **Why simulate?** — Cheaper, repeatable experiments on scheduling/cost before using a real cloud.
