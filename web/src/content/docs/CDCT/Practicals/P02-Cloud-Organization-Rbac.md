---
title: "P02 — Cloud Organization Rbac"
sidebar:
  order: 2
---
...
1. `IAM → Policies → Create policy → JSON` → paste the JSON from [`p02_iam_policy.json`](./p02_iam_policy.json.md).
...
See full file: [`p02_iam_policy.json`](./p02_iam_policy.json.md)
...
> When running **Cloud Organization Rbac** in a real environment, it almost never works perfectly the first time. 
> 
...
- **AWS IAM terms:** — **Root user:** account owner; full access; should be used only for account setup (MFA mandatory).
- **What is RBAC?** — Granting permissions based on roles; users are mapped to roles/groups.
- **Root user vs IAM user?** — Root = account owner (MFA only); IAM users get least-privilege scoped access.
- **Why use groups instead of per-user policies?** — Single source of truth; a new employee inherits the right permissions instantly.
- **What does an explicit Deny do?** — Overrides any Allow; used to block dangerous actions (IAM changes) even for powerful roles.
- **IAM vs SCP?** — IAM governs what a *user/role* can do; SCP governs what a *child account* can do in an organization.
