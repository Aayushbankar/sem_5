---
title: "P09 — Minio Secure Object Storage"
sidebar:
  order: 9
---
...
| File | Purpose |
|---|---|
| [`p09_docker-compose.yml`](./p09_docker-compose.yml.md) | MinIO server (ports 9000 API / 9001 console) + KMS key |
| [`p09_readonly_policy.json`](./p09_readonly_policy.json.md) | IAM policy: `GetObject`, `ListBucket` (read-only) |
| [`p09_admin_policy.json`](./p09_admin_policy.json.md) | IAM policy: `PutObject`, `GetObject`, `DeleteObject` |
| [`p09_bucket_policy.json`](./p09_bucket_policy.json.md) | Example *bucket* policy (AWS-style) for reference |
| [`p09_setup.sh`](./p09_setup.sh.md) | End-to-end setup + verification script |
...
> When running **Minio Secure Object Storage** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Ran for real in this environment** — Docker + MinIO were available. The console output below is the **actual captured run**.
- **What is MinIO?** — Open-source S3-compatible object storage server.
- **What is SSE-S3?** — Server-side encryption where the server encrypts objects with AES-256; no client key management.
- **How is access controlled?** — IAM users + JSON policies (`Effect`/`Action`/`Resource`); least privilege.
- **Why is encryption at rest important?** — Protects data if disks/backups are stolen; satisfies compliance.
- **MinIO vs AWS S3?** — Same S3 API; MinIO runs on your own hardware (private cloud / on-prem).
