...
> Contract file: [`P08_GreenEnergyToken.sol`](./P08_GreenEnergyToken.sol.md)
...
> When running **Erc20 Token** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Green Energy Token (GET)** — 1 token = 1 kWh of green energy, tradeable on-chain.
- **Difference between ETH and an ERC-20 token?** — ETH is native protocol asset (pays gas); ERC-20 is a smart contract implementing a standard.
- **Why use `transferFrom`?** — Lets a third party (e.g., a DEX contract) spend on your behalf after approval.
- **What is an allowance?** — The amount a spender is authorized to pull from an owner's balance.
- **Why 18 decimals?** — Convention to avoid precision loss (mirrors wei).
- **What is a "standard" like ERC-20?** — An interface definition (EIP) that wallets/exchanges can rely on.
