...
> Full runnable script: [`p04_merkle_tree.py`](./p04_merkle_tree.py.md)
...
> When running **Merkle Tree** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Simplified Payment Verification (SPV)** — light wallets store only the block header (root) and verify a transaction with a short proof.
- **Why duplicate the last leaf when the count is odd?** — So every parent has two children (Balanced binary tree).
- **What is a Merkle proof?** — The path of sibling hashes from a leaf to the root used to verify membership.
- **Why not just hash all transactions together?** — Then verifying one tx requires all txs; a Merkle tree only needs the branch.
