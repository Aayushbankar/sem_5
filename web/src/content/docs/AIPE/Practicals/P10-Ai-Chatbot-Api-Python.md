---
title: "P10 — Ai Chatbot Api Python"
sidebar:
  order: 10
---
...
Script: [`p10_chatbot.py`](./p10_chatbot.py.md) — key parts:
...
- Script: [`p10_chatbot.py`](./p10_chatbot.py.md)
...
> When running **Ai Chatbot Api Python** in a real environment, it almost never works perfectly the first time. 
> 
...
- **REST APIs** — you send a JSON `POST` request with your prompt, and get back the model's generated text.
- **What is an API key?** — A secret credential that identifies you to the provider; sent in the Authorization header.
- **Why read the key from an environment variable?** — So it never lives in source code and can't be committed to git.
- **What is the endpoint?** — The URL your client POSTs the request to.
- **What is a system message/role?** — The role that sets the model's behaviour ("you are a study assistant").
- **What is `--mock` for?** — Running the full loop offline with canned answers, for demos/tests with no key.
- **Which header carries the key?** — `Authorization: Bearer <key>`.
