---
title: "P04 — Llm Capabilities Limitations Hallucinations"
sidebar:
  order: 4
---
...
- Question bank + rubric: [`p04_question_bank_and_rubric.md`](./p04_question_bank_and_rubric.md.md)
...
> When running **Llm Capabilities Limitations Hallucinations** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Capabilities of LLMs:** — Answer factual questions, summarize, translate, generate code.
- **invents sources** — look up any cited paper/book/URL.
- **What is a hallucination?** — A confident, factually wrong statement generated by the model.
- **What is the context window?** — The max tokens a model can process in one request.
- **Why do LLMs hallucinate?** — They predict plausible tokens, not verified facts.
- **How do you reduce hallucination in practice?** — Grounding via RAG (P11), instructing "answer only from the text", asking the model to state uncertainty.
- **What is bias in AI?** — Systematic unfairness learned from skewed training data.
- **Why is the cost a limitation?** — API usage bills per input/output token; huge contexts and repeated calls add up.
