---
title: "P11 — Document Qa Basic Rag"
sidebar:
  order: 11
---
...
Script: [`p11_rag_document_qa.py`](./p11_rag_document_qa.py.md), sample doc: [`p11_sample_document.txt`](./p11_sample_document.txt.md)
...
- Script + sample doc: [`p11_rag_document_qa.py`](./p11_rag_document_qa.py.md) · [`p11_sample_document.txt`](./p11_sample_document.txt.md)
...
> When running **Document Qa Basic Rag** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Swap the document** — any `.txt` becomes a Q&A subject.
- **What does RAG stand for and what does it add?** — Retrieval Augmented Generation; it adds external documents as context to the model.
- **What is a chunk?** — A small overlapping passage of the document; the retrieval unit.
- **What is an embedding?** — A vector of numbers representing a text's meaning.
- **How are chunks ranked?** — Cosine similarity between question vector and chunk vector.
- **Why does RAG reduce hallucinations?** — The answer is grounded in the retrieved text instead of the model's memory.
- **What role does P10's ChatClient play?** — It is reused to call the API (or mock) — same client, new application.
