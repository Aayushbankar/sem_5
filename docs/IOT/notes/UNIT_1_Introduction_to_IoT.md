...
> **Related practicals:** [P01](../practicals/writeups/P01_iot_architecture_layers.md)
...
flowchart TD
    subgraph APP[4. Application Layer]
        ...
        ALERT[SMS / push alerts]
    end
    subgraph PROC[3. Data Processing Layer]
        CLOUD[Cloud platform - ThingSpeak / Blynk]
        RULE[Threshold rules & analytics]
    end
    subgraph NET[2. Network Layer]
        WIFI[ESP32 Wi-Fi / BLE / Zigbee]
        GW[Gateway & internet]
    end
    subgraph SENSE[1. Sensing Layer]
        S1[DHT11]
        S2[PIR]
        S3[HC-SR04]
        ACT[Relay / LED]
...
> 💡 **Exam link:** REST = "ask and get an answer" (P10's `?api_key=…&field1=27.4`); MQTT = "subscribe and receive forever" (P09's remote LED). Compare them on **latency, bandwidth, and connection model** — that is the 4-marker.
...
```mermaid
flowchart LR
    CH[Challenges] --> D[Design<br/>heterogeneity, power, scale]
    CH --> S[Security<br/>privacy, botnets, tampering]
    CH --> P[Deployment<br/>reliability, OTA, compliance]
```
...
| 🛠️ **Builder** | wiring things yourself | Watch the intro → jump to [P01](../practicals/writeups/P01_iot_architecture_layers.md) and map real sensors to layers |
...
1. **"NPTEL — Introduction to Internet of Things"** (the exact MOOC recommended in the syllabus) — full university course covering this unit's architecture and challenges.
2. **"edureka! IoT full course / IoT tutorial for beginners"** — broad, visual overview of every layer and protocol.
3. **"Random Nerd Tutorials — ESP32 getting started playlist"** — the *hands-on* follow-up that bridges this unit to the hardware in Unit 3.

### 🎬 Step 3 — Proof you got it (5 min)

- Can you point at 3 objects in your room and name the layer each occupies in a 4-layer diagram?
- Explain the difference between M2M and IoT to a friend using one sentence.
- List 3 security problems of a smart home *without* looking — if you can, the challenges section is yours.

---

*Next: [UNIT 2 — IoT Sensors, Actuators and Hardware Platforms](./UNIT_2_IoT_Sensors_Actuators_Hardware_Platforms.md)*
