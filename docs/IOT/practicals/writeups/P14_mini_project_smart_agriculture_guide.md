...
> ⚠️ **Pump safety:** the pump is powered **separately** (12 V supply), never from the ESP32's 5 V pin. The relay's switch contacts (COM → NO) complete the pump's power path; the ESP32 only drives the relay coil.
...
> Full sketch: [`p14_smart_agriculture_project.ino`](../code/p14_smart_agriculture_project.ino)
