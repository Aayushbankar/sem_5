...
> 💡 **Exam one-liner:** the same ESP32 node plays two roles — in the **sensing layer** it reads the sensor, and in the **network layer** it transmits. This is why ESP32 is called an "IoT-ready" microcontroller (built-in Wi-Fi/BT).
...
> 💡 **IIoT vs Smart Home difference to quote in viva:** IIoT prioritises **reliability, low latency and deterministic behaviour** (a robot cannot wait 500 ms for a command), so it uses wired/industrial protocols (Modbus, PROFINET) and **edge processing**; Smart Homes tolerate internet latency because comfort, not safety, is at stake.
