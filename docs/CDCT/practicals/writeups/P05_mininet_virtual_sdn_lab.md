...
> ⚠️ **Lab environment note:** This machine does **not** have Mininet installed (no network namespaces / Open vSwitch). The steps and expected output below are documented from the official Mininet workflow and must be run **in the lab VM** (`sudo apt install -y mininet`).
...
flowchart TB
    ...
    S2 --- H4[h4 10.0.0.4]
```
This is exactly the topology in [`p05_mininet_2switch_4host.py`](../code/p05_mininet_2switch_4host.py).
...
File: [`p05_mininet_2switch_4host.py`](../code/p05_mininet_2switch_4host.py)
