...
> When running **Cloud Storage Comparison** in a real environment, it almost never works perfectly the first time. 
> 
...
- **Object vs block vs file storage?** — Object: flat key-value, HTTP API, metadata; Block: raw volumes mounted by VMs; File: NFS/SMB shares.
- **What is durability?** — Probability data survives (11 nines = extremely high) via replication/erasure coding.
- **What are storage classes?** — Tiers trading cost for access frequency (Standard → Archive).
- **Is S3 strongly consistent?** — Yes since December 2020: reads reflect the latest write.
- **What is multipart upload?** — Uploading large objects in parts in parallel; speeds up big transfers.
