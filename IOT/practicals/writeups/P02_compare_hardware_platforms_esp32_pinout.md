...
> 💡 **Exam one-liner to memorise:** Arduino UNO = *simple digital/analog control, no networking*; ESP32 = *networking-first microcontroller (Wi-Fi + BT on-chip)*; Raspberry Pi = *a Linux computer with GPIO*, used as a gateway/edge device, not a bare-metal controller.
...
> ⚠️ **Pin safety rules (viva favourite):**
...
> 💡 **Arduino practical benefit:** in the Arduino IDE you don't manage cores directly — `setup()`/`loop()` run on Core 1 while the Wi-Fi library quietly uses Core 0. If you ever need true parallelism you can call `xTaskCreatePinnedToCore()` (FreeRTOS).
